...
class ReferencedRepositoryUpdatedDetails(BaseModel):
    """
    ...
    repository: StrictStr = Field(description="The name of the repository that was updated.")
    __properties: ClassVar[List[str]] = ["detailType", "repository"]

    @field_validator('detail_type')
    def detail_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ReferencedRepositoryUpdated']):
            raise ValueError("must be one of enum values ('ReferencedRepositoryUpdated')")
        return value
