...
class FleetRolloutCompletedDetails(BaseModel):
    """
    ...
    template_version: StrictStr = Field(description="The name of the TemplateVersion that this fleet rollout is completed for.", alias="templateVersion")
    __properties: ClassVar[List[str]] = ["detailType", "templateVersion"]

    @field_validator('detail_type')
    def detail_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FleetRolloutCompleted']):
            raise ValueError("must be one of enum values ('FleetRolloutCompleted')")
        return value
