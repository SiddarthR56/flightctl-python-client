...
class DeviceMultipleOwnersDetectedDetails(BaseModel):
    """
    ...
    matching_fleets: List[StrictStr] = Field(description="List of fleet names that match the device.", alias="matchingFleets")
    __properties: ClassVar[List[str]] = ["detailType", "matchingFleets"]

    @field_validator('detail_type')
    def detail_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DeviceMultipleOwnersDetected']):
            raise ValueError("must be one of enum values ('DeviceMultipleOwnersDetected')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "matchingFleets": obj.get("matchingFleets")
        })
        return _obj
