...
class FleetRolloutBatchDispatchedDetails(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["detailType", "templateVersion", "batch"]

    @field_validator('detail_type')
    def detail_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FleetRolloutBatchDispatched']):
            raise ValueError("must be one of enum values ('FleetRolloutBatchDispatched')")
        return value
