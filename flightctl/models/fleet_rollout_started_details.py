...
class FleetRolloutStartedDetails(BaseModel):
    """
    ...
    template_version: StrictStr = Field(description="The name of the TemplateVersion that is rolling out.", alias="templateVersion")
    rollout_strategy: StrictStr = Field(description="Rollout strategy type.", alias="rolloutStrategy")
    __properties: ClassVar[List[str]] = ["detailType", "templateVersion", "rolloutStrategy"]

    @field_validator('detail_type')
    def detail_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FleetRolloutStarted']):
            raise ValueError("must be one of enum values ('FleetRolloutStarted')")
        return value

    @field_validator('rollout_strategy')
    def rollout_strategy_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['None', 'Batched']):
            raise ValueError("must be one of enum values ('None', 'Batched')")
        return value
