...
class FleetRolloutDeviceSelectedDetails(BaseModel):
    """
    ...
    fleet_name: StrictStr = Field(description="The name of the fleet that the device is being selected for.", alias="fleetName")
    template_version: StrictStr = Field(description="The name of the TemplateVersion that the device is being selected to render.", alias="templateVersion")
    __properties: ClassVar[List[str]] = ["detailType", "fleetName", "templateVersion"]

    @field_validator('detail_type')
    def detail_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FleetRolloutDeviceSelected']):
            raise ValueError("must be one of enum values ('FleetRolloutDeviceSelected')")
        return value
