...
class DeviceOwnershipChangedDetails(BaseModel):
    """
    ...
    previous_owner: Optional[StrictStr] = Field(default=None, description="The previous owner fleet (null if none).", alias="previousOwner")
    new_owner: Optional[StrictStr] = Field(default=None, description="The new owner fleet (null if removed).", alias="newOwner")
    __properties: ClassVar[List[str]] = ["detailType", "previousOwner", "newOwner"]

    @field_validator('detail_type')
    def detail_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DeviceOwnershipChanged']):
            raise ValueError("must be one of enum values ('DeviceOwnershipChanged')")
        return value
