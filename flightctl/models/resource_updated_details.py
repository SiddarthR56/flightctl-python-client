...
class ResourceUpdatedDetails(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["detailType", "updatedFields", "previousOwner", "newOwner"]

    @field_validator('detail_type')
    def detail_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ResourceUpdated']):
            raise ValueError("must be one of enum values ('ResourceUpdated')")
        return value
    ...
    def updated_fields_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['owner', 'labels', 'spec', 'spec.selector', 'spec.template']):
                raise ValueError("each list item must be one of ('owner', 'labels', 'spec', 'spec.selector', 'spec.template')")
        return value
